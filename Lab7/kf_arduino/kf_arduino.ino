...
void foward(int speed) {
  ...
  analogWrite(AB1IN_RIGHT,speed*1.3); 
...
void backward(int speed) {
  ...
  analogWrite(AB2IN_RIGHT,speed*1.3);
}
void stop() {
  analogWrite(AB1IN_LEFT,0); 
  analogWrite(AB2IN_LEFT,0);
...
float pid_orient_imu() {
  ...
  if (pid_orien_speed > 0) {
    ...
    else if (pid_orien_speed < min_turn_speed) {
      pid_orien_speed = min_turn_speed;
    }
    cw(pid_orien_speed, 1.5); // motors turning speed adjustment constant 1.5 when cw
  }
  else {
    if (pid_orien_speed < (-1 * max_turn_speed)) {
      pid_orien_speed = -1 * max_turn_speed;
    }
    else if (pid_orien_speed > (-1 * min_turn_speed)) {
      pid_orien_speed = -1 * (min_turn_speed);
    }
    ccw(-1 * pid_orien_speed, 1.8); // motors turning speed adjustment constant 1.8 when ccw
  }
  prev_time = millis();
...
void loop() {
  ...
  if (central) {  // connected
    ...
    if (central.connected()) {
      ...
      // }
      // else if (cmd_type == 5) {
      //   turn_left(turn_speed, turn_dif);
      // }
      // else if (cmd_type == 6) {
      //   turn_right(turn_speed, turn_dif);
      // }
      kf_record();
    }
    else {
