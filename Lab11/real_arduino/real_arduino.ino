...
float pid_orient_imu() {
  ...
  if (pid_orien_speed > 0) {
    ...
    cw(pid_orien_speed, pid_turn_diff); // motors turning speed adjustment constant 1.5 when cw
  }
  else {
    ...
    ccw(-1 * pid_orien_speed, pid_turn_diff); // motors turning speed adjustment constant 1.8 when ccw
  }
  prev_time = millis();
...
void local_data_store() {
  if (recording) {
    ...
    if (angle_count >= 19 || millis() - recording_start_time >= recording_duration) {
        recording = false;
        stop();
        for (int i = 0; i < local_data_index; i++) {
          tx_estring_value.clear();
