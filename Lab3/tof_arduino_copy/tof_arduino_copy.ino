...
void imu_data_store() {
  if (recording) {
    ...
    if (millis() - recording_start_time >= recording_duration) {
        ...
        Serial.print(" total imu data points collected in ");
        Serial.print(recording_duration/1000);
        Serial.print(" second, in ");
        Serial.print(loop_count);
        Serial.println(" total loop iterations. ");
    }
    IMUDataReady = false;
  }
  ...
  if (imu_data_count > 0) {  // if there is still data need to be send
    ...
    Serial.print(total_imu_data_count);
    Serial.print(" total imu data points sent in ");
...
void tof_data_store() {
  if (recording) {
    ...
    if (millis() - recording_start_time >= recording_duration) {
        ...
        Serial.print(" total tof data points collected in ");
        Serial.print(recording_duration/1000);
        Serial.print(" second, in ");
        Serial.print(loop_count);
        Serial.println(" total loop iterations. ");
  ...
  // tof1 store data
  if (tof_data_count > 0) {  // if there is still data need to be send
      if (tof_data_full) {  // if the first array has value to send
        tx_estring_value.clear();
        ...
        tx_characteristic_string.writeValue(tx_estring_value.c_str());
        tof_data_count--;  // indicate one line of data has sent
      }
      // send previous data before replacing with new data
      timestamps[tof_data_index] = millis();
      ...
      tofDataReady = true;
      if (tof_data_index >= NUM_SAMPLES) {  // if last element is stored, indicate full and use array2
          tof_data_full = true;
          tof_data_index = 0;
      }
  }

  else if (total_tof_data_count && tof_finish == false) {
    Serial.print(total_tof_data_count);
    Serial.print(" total tof data points sent in ");
    int end_time = (millis() - recording_start_time)/1000;
    Serial.print(end_time);
    Serial.println(" second. ");
    tof_finish = true;
  ...
  if (!recording && tof_data_index > 0 && !tof_finish) {
    for (int i = 0; i < tof_data_index; i++) {
      tx_estring_value.clear();
      tx_estring_value.append("ToF: ");
...
void loop() {
  ...
  if (central) {  // connected
    ...
    if (central.connected()) {
      ...
      // check for imu data
      if (myICM.dataReady() && recording && IMUDataReady) {  
        myICM.getAGMT();
        IMUDataReady = false;
      }
      else if (!IMUDataReady){
        imu_data_store();
      }
      ...
      tof1.startRanging(); 
      tof2.startRanging(); 
      if (tof1.checkForDataReady() && recording && tofDataReady) {
        tofDataReady = false;
      }     
      else if (!tofDataReady) {
        tof_data_store();
      } 
    }
    else {
